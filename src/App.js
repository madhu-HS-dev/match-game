...
class App extends Component {
  ...
  onClickGameItem = id => {
    ...
    const filteredImage = filteredList[0].imageUrl
    const randomNumber = Math.floor(Math.random() * imagesList.length - 1)

    if (filteredImage === imgUrl) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgUrl: imagesList[randomNumber].imageUrl,
      }))
    } else {
      this.setState({isTrue: false})
      clearInterval(this.timerId)
  ...
  renderActiveGameContainer = () => {
    const {activeTabId, imgUrl} = this.state
    ...
    return (
      <div className="game-container">
        <img src={imgUrl} alt="match" className="image-url" />
  ...
  render() {
    const {timer, score, isTrue} = this.state
    ...
    return (
      <div className="match-game-container">
        ...
        <div className="bottom-container">
          {isTrue
            ? this.renderActiveGameContainer()
            : this.renderGameOverContainer()}
        </div>
      </div>
    )
  }
